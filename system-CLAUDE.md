...
3. `npm test` (Jest) — covers `combat-math.js`, `char-math.js`, `roll-math.js`, and `tests/extension-hooks.test.js`'s mirror-style coverage of Foundry-coupled `CombatEngine`/`CharacterData` call sites (see that file's header for the mirroring convention). The suite's test count changes every batch — don't hardcode it here, just keep it green. `jest.fn`/mock helpers are **not available** in this project's ESM Jest setup (`--experimental-vm-modules`, no global injection) — hand-roll a spy (record calls in a closure array) instead of reaching for `jest.fn()`. Keep the suite green; add tests for new pure logic.
...
- **A rules "next turn" clause on a multi-part Special Effect binds to whichever sub-action actually costs a turn — not to the whole SE.** Impale (rules p.44) shipped this wrong for a long time (fixed v1.4.294): "the attacker has the option of leaving the weapon in the wound or yanking it free on their next turn" only gates *yanking* (a Ready Weapon + Brawn action); leaving it in is the default state from the instant the wound occurs, with no action cost and thus no reason to wait. The old code applied the `impaledBy` penalty flag only when the attacker clicked a decision-card button that itself didn't appear until `updateCombat` fired on their next turn — a full-turn gap where the defender (and `DefenderDialog.js`'s "can't parry your own impaled weapon" check) saw no penalty at all. Before deferring any SE's *state* to a "next turn" decision card, check whether the state itself has an action cost or whether only one branch of the SE does — only the action-gated branch should wait.
- **A full audit of all 32 rulebook Special Effects against their code is DONE (v1.4.295)** — 29 matched RAW exactly, 3 gaps found and fixed: Arise was entirely missing from the SE registry (added, `arise.js`); Trip Opponent never implemented the Quadruped substitute-Athletics-for-Evade/one-grade-easier rule (added, gated on a new `quadruped` `creatureTraits` entry); Slip Free never cleared Pin Weapon/Pin Object state alongside Grip/Entangle. See `CHANGELOG.md` v1.4.295 for the full account, including two low-priority doc-accuracy notes (not fixed) about `pin-weapon.js`'s dubious "Rules p.45" citation and Circumvent Cover's genuinely missing rulebook prose.
- **An SE resolver that needs to READ an actor's current status (not just write one) must resolve the canvas token first, the same way `applyStatusToActor`/`removeStatusFromActor` already do internally — don't read `actor.statuses` straight off whatever `ctx.defender`/`ctx.attacker` happens to be.** In real combat flow `ctx.defender` is already `token.actor` (`CombatEngine._resolveOpposedSEs` builds it that way), so this usually doesn't bite — but `SE_RESOLVERS` is also documented as "the node editor's SE entry point," and any caller that builds `ctx` from `game.actors.get(...)` instead of a live token passes the *base* actor, which for an unlinked token is a different object than the one `toggleStatusEffect` actually modifies. Caught live (v1.4.295): `resolveArise`'s first version checked `defender.statuses.has('prone')` directly and silently no-op'd — passed every code review and `node --check`, only failed once actually exercised end-to-end against the running world. `applyStatusToActor`/`removeStatusFromActor` themselves are already safe (they re-resolve via `canvas.tokens.placeables.find(t => t.actor?.id === actor.id)`, matching by id regardless of link status) — the bug is only in code that reads `.statuses` *before* calling them.
...
`git add .` → `git commit -m "v1.4.xxx - description"` → `git push` after each stable, tested batch. **Plain hyphens in commit messages** — em dashes cause terminal hangs. Bump the version in `system.json` only — **`package.json` has no `version` field and hasn't since v1.4.279** (deliberately removed, "one authoritative version location, not two to remember," after drifting 5 versions behind `system.json` once — see that version's `CHANGELOG.md` entry). This paragraph used to say to bump both; that instruction is what drifted, not the code — corrected here rather than left for the next person to rediscover.

## Live testing

Local dev instance at `C:/Users/Mahle/AppData/Local/FoundryVTT/Data/` (corrected — an old `Mahlers7th` path here was stale, doesn't match this machine) — **separate** from the Sunday game (Molten hosting, still v13). GM Mode on, semi-auto automation. Errors from the running instance (console + exported item JSON) are ground truth over static analysis.

**Never delete an active module's files from `Data/modules/` without disabling it in the world first** (`game.settings.get('core','moduleConfiguration')`, flip its key `false`, `game.settings.set(...)`, and let a fresh page load confirm `game.modules.get(id).active === false` before touching disk). Deleting files out from under an *active* module doesn't just stop it cleanly — any World/Actor/Item documents using a type that module's `documentTypes`/`CONFIG.*.dataModels` declared become permanently un-loadable ("not a valid type for the X Document class" on every future boot), and at that point `Document.deleteDocuments(ids)` silently no-ops (`deletedCount: 0`) because the client can't construct an invalid document to route the delete through — even though the server-side delete request usually still lands. Recovered once already (removing the `spike-a2-typetest` dev spike, 2026-08): had to temporarily recreate a minimal stub module (empty `TypeDataModel`, same id/type keys) just to let the orphaned documents load again, delete them properly, then remove the stub. Disable-first avoids the whole detour.
