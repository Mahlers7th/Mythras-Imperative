...
| **`bashKnockbackMultiplierHooks`** (v1.4.270) | `(attacker, weapon) => number` | Read-time, consulted once per Bash resolution | `module/combat/effects/bash.js` (`:52`), immediately before the knockback-distance formula | Default multiplier `1`. **Last-valid-wins, NOT multiplicative** — each hook's result, if `Number.isFinite(result) && result > 0`, directly **overwrites** `knockbackMultiplier` (a plain assignment, not `*=`); a later valid hook silently replaces an earlier one rather than compounding with it. (`config.js`'s own inline comment for this hook currently claims "Multiple hooks would multiply together" — confirmed wrong against `bash.js`'s actual loop while writing this doc; flagged for a future code-adjacent doc fix, not corrected here since this batch is `.md`/`package.json` only.) A non-finite or non-positive result is silently ignored, loop continues. See dedicated section below. |
| **`evasionHooks`** | `(ctx, applyProne) => boolean \| undefined` | Attack-resolution lifecycle event, Step 8 of `_afterDefenceResolved` | `CombatEngine._afterDefenceResolved` (`:949`), only when `ctx.defenceType` is `'evade'` or `'acrobatics'` | Previously entirely undocumented — a real, exercised hook, not vestigial. `applyProne` starts as `ctx.willBeProne ?? false` (the engine's own prior determination). A hook returning **exactly `false`** clears `applyProne` and the loop `break`s immediately — no further hook is consulted once one cancels it. Any other return (`true`, `undefined`, anything else) is ignored: **this hook can only CANCEL an engine-decided prone result, never FORCE one the engine didn't already decide on** — an asymmetric, one-directional contract, not the boolean-override shape its signature might suggest at a glance. **No try/catch around this loop** — the one hook family in this file with no error isolation; a throwing hook here propagates uncaught out of `_afterDefenceResolved`, unlike every other hook array documented in this file. No registered consumers anywhere as of this writing (checked the Destined module) — live, real infrastructure, simply unused so far. See dedicated section below. |
