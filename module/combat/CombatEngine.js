...
export class CombatEngine {
  ...
  // -------------------------------------------------------------------------
  static async _resolveBleed(ctx, damage, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    ...
    if (bleedApplied) {
      await CombatEngine._applyStatusToActor(defender, 'bleeding');
    }
  ...
  // -------------------------------------------------------------------------
  static async _resolveTripOpponent(ctx, damage, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    const isGMMode = CombatEngine.gmMode;
    const attackRoll = ctx.attackResult ?? 0;
    ...
    const _adj = raw => CombatEngine._applyFatigueToSkill(raw, resistingActor);
    const skillOptions = [
    ...
    if (tripApplied) {
      await CombatEngine._applyProneToDefender(resistingActor);
    }
  ...
  // -------------------------------------------------------------------------
  static async _resolveStunLocation(ctx, damage, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    const isGMMode = CombatEngine.gmMode;
    const attackRoll = ctx.attackResult ?? 0;
    ...
    if (stunApplied) {
      ...
      const locationType = ctx.locationType ?? CombatEngine._classifyLocation(ctx.hitLocationLabel ?? '');
      if (locationType === 'torso') {
        ...
        if (fallsProne) {
          await CombatEngine._applyProneToDefender(defender);
        }
  ...
  // -------------------------------------------------------------------------
  static async _resolveDisarmOpponent(ctx, damage, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    const isGMMode = CombatEngine.gmMode;
    const attackRoll = ctx.attackResult ?? 0;
    ...
    let   resistSkillTotal = CombatEngine._applyFatigueToSkill(resistSkillRaw, resistingActor);
  ...
  static async _resolveBlindOpponent(ctx) {
    const { attacker, defender } = ctx;
    ...
    const resistSkillTotal = CombatEngine._applyFatigueToSkill(resistSkillRaw, attacker);
    ...
    if (forcesFail) {
      ...
      attackerSucceeds = CombatEngine._resolveOpposedRoll(
    ...
    if (blindApplied) {
      // Apply token status
      await CombatEngine._applyStatusToActor(attacker, 'blinded');
  ...
  static async _resolveDropFoe(ctx, damage, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    const isGMMode = CombatEngine.gmMode;
    const attackRoll = ctx.attackResult ?? 0;
  ...
  static async _resolvePinDown(ctx, forcesFail) {
    const { attacker, defender } = ctx;
    const isSemi   = CombatEngine.automationLevel === 'semi';
    const isGMMode = CombatEngine.gmMode;
    const attackRoll = ctx.attackResult ?? 0;
    ...
    const wpTotal = CombatEngine._applyFatigueToSkill(wpRaw, defender);
  ...
  // -------------------------------------------------------------------------
  static _waitForCard(msgId, ms = 800) {
  ...
  static async _runSEDialog(data) {
    ...
    await CombatEngine._waitForCard(data.lastCardId);
          ...
          buttons: {
            roll: {
              ...
              callback: async () => {
                ...
                const roll = new Roll('1d100');
                await roll.evaluate();
                const succeeds = CombatEngine._resolveOpposedRoll(
    ...
    if (seType === 'woundEndurance') {
      return CombatEngine._runWoundEnduranceDialog(data);
    }
  ...
  static async _runWoundEnduranceDialog({
  ...
  static async _postOpposedSEResult({
  ...
  static _classifyLocation(locationName) {
   ...
   */
  static async _applyProneToDefender(defender) {
    await CombatEngine._applyStatusToActor(defender, 'prone');
   ...
   */
  static async _applyStatusToActor(actor, statusId) {
  ...
  static async _removeStatusFromActor(actor, statusId) {
  ...
  static _applyFatigueToSkill(skillTotal, actor) {
    ...
    const entangleGrade = CombatEngine._getActiveEntangleGrade(actor);
    if (entangleGrade) {
  ...
  // -------------------------------------------------------------------------
  static _getActiveImpaleGrade(actor) {
  ...
  // -------------------------------------------------------------------------
  static _getActiveEntangleGrade(actor) {
    if (!actor) return null;
